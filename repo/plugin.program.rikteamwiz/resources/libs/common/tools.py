...
def kill_kodi(msg=None, over=None):
    if over:
        ...
        if not msg:
            msg = '[COLOR {0}]Se va a forzar el cierre de Kodi, ¿Quieres Salir?[/COLOR]'.format(CONFIG.COLOR2)
            ...
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
                ...
                if progress_dialog.iscanceled():
                    ...
                    logging.log_notify(CONFIG.ADDONTITLE,
                                       "[COLOR {0}]Convertir Directorios Cancelado[/COLOR]".format(CONFIG.COLOR2))
                    sys.exit()
    progress_dialog.close()
    logging.log("[Convert Paths to Special] Complete")
    if not over:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Convertir Directorios Completado[/COLOR]".format(CONFIG.COLOR2))
...
def reload_fix(default=None):
    ...
    dialog.ok(CONFIG.ADDONTITLE,
                  "[COLOR {0}]ADVERTENCIA: A veces la recarga del perfil hace que Kodi se bloquee. Mientras Kodi esta recargando el perfil, no pulse ningun boton.[/COLOR]".format(CONFIG.COLOR2))
...
def ascii_check(use=None, over=False):
    ...
    if use is None:
        ...
        if over:
            ...
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]¿Desea [COLOR {1}]borrar[/COLOR] todos los nombres de archivo con caracteres especiales o prefiere solo [COLOR {2}]escanear y ver[/COLOR] los resultados en el registro?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1, CONFIG.COLOR1),
                                   yeslabel='[B][COLOR springgreen]Borrar[/COLOR][/B]',
    ...
    if source == "":
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Comprobar ASCII: Cancelado[/COLOR]".format(CONFIG.COLOR2))
        return
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, 'Por favor, espera...')
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            progress_dialog.update(prog2, "[COLOR {0}]Comprobando que no haya archivos ASCII".format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file) + '\n' + 'Por favor, espera[/COLOR]')
            try:
                file.encode('ascii')
        ...
        if progress_dialog.iscanceled():
            progress_dialog.close()
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Comprobacion ASCII Cancelada[/COLOR]".format(CONFIG.COLOR2))
            sys.exit()
    progress_dialog.close()
    afiles.close()
    afails.close()
    ...
    if total > 0:
        ...
        if yes:
            if use:
                logging.log_notify(CONFIG.ADDONTITLE,
                                 "[COLOR {0}]Comprobacion ASCII: {1} Eliminado / {2} Failed.[/COLOR]".format(CONFIG.COLOR2, f1, f2))
            else:
                window.show_text_box("Viendo Archivos ASCII Eliminados",
                                  "[COLOR yellow][B]{0} Archivos Eliminados:[/B][/COLOR]\n {1}\n\n[COLOR yellow][B]{2} Archivos Fallidos:[B][/COLOR]\n {3}".format(f1, msg, f2, msg2))
        else:
            window.show_text_box("Viendo Archivos ASCII Encontrados", "[COLOR yellow][B]{0} Archivos Encontrados:[/B][/COLOR]\n {1}".format(f1, msg))
    else:
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Comprobacion ASCII: Ninguno Encontrado.[/COLOR]".format(CONFIG.COLOR2))
...
def open_url(url, stream=False, check=False, cred=None, count=0):
    ...
    if not valid:
        ...
        if response.status_code == 401:
            retry = dialog.yesno(CONFIG.ADDONTITLE, 'El nombre de usuario o la contraseña no son validos. ¿Quiere volver a intentarlo?', yeslabel='SI', nolabel='NO')
            ...
            if retry and count < 3:
                ...
                dialog.ok(CONFIG.ADDONTITLE, 'Autentificacion Fallida.')
                return False

...
def wipe():
    ...
    if CONFIG.KEEPalfa == 'true':
        ...
        CONFIG.set_setting('alfanextsave', str(tools.get_date(days=3, formatted=True)))
    ...
    skin.skin_to_default('Instalacion Limpia')
    
    update.addon_updates('set')
    xbmcPath = os.path.abspath(CONFIG.HOME)
    progress_dialog.create(CONFIG.ADDONTITLE, "[COLOR {0}]Calculando Archivos".format(CONFIG.COLOR2) + '\n' + '\n' + 'Espere![/COLOR]')
    ...
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        ...
        for name in files:
            ...
            if name == 'sources.xml' and fold[-1] == 'userdata' and CONFIG.KEEPSOURCES == 'true':
                logging.log("Mantener sources.xml: {0}".format(os.path.join(root, name)))
            elif name == 'MyVideos119.db' and fold[-1] == 'Database' and CONFIG.KEEPMYVIDEOS == 'true':
                logging.log("Mantener MyVideos119.db: {0}".format(os.path.join(root, name)))
            elif name == 'favourites.xml' and fold[-1] == 'userdata' and CONFIG.KEEPFAVS == 'true':
                logging.log("Mantener favourites.xml: {0}".format(os.path.join(root, name)))
            elif name == 'profiles.xml' and fold[-1] == 'userdata' and CONFIG.KEEPPROFILES == 'true':
                logging.log("Mantener profiles.xml: {0}".format(os.path.join(root, name)))
                ...
                logging.log("Mantener Log File: {0}".format(name))
        ...
        if progress_dialog.iscanceled():
            ...
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Instalacion Limpia Cancelada[/COLOR]".format(CONFIG.COLOR2))
            return False
    for root, dirs, files in os.walk(xbmcPath, topdown=True):
        dirs[:] = [d for d in dirs if d not in exclude_dirs]
        for name in dirs:
            progress_dialog.update(100, '\n' + 'Limpieza de carpeta vacía: [COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name))
...
def fresh_start(install=None, over=False):
    ...
    if yes_pressed:
        ...
        if over:
            ...
            dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Para aplicar los cambios se requiere forzar el cierre de la aplicación, pulsa OK para forzarlo[/COLOR]".format(CONFIG.COLOR2))
            from resources.libs import update
    ...
    if tools.platform() == 'android':
        ...
        if os.path.exists(lib):
            ...
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Quieres Descargar el ".format(CONFIG.COLOR2),
            ...
            if not yes:
                logging.log_notify(CONFIG.ADDONTITLE,
                               '[COLOR {0}]EROR: Instalacion Cancelada[/COLOR]'.format(CONFIG.COLOR2))
                return
        ...
        dialog.ok(CONFIG.ADDONTITLE, '[COLOR {}]{}[/COLOR] descargado a [COLOR {}]{}[/COLOR]. If installation doesn\'t start by itself, navigate to that location to install the APK.'.format(CONFIG.COLOR1, apk, CONFIG.COLOR1, path))

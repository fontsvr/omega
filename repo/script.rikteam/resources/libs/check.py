...
def check_build_update():
    ...
    if len(match) > 0:
        ...
        icon = match[0][1]
        fanart = match[0][2]
        CONFIG.set_setting('ultima versión', version)
        if version > CONFIG.BUILDVERSION:
            if CONFIG.DISABLEUPDATE == 'false':
                logging.log("[Check Updates] [Version Instalada: {0}] [Version Anterior: {1}] Abrir ventana de actualización".format(CONFIG.BUILDVERSION, version))
                window.show_update_window(CONFIG.BUILDNAME, CONFIG.BUILDVERSION, version, icon, fanart)
            else:
                logging.log("[Check Updates] [Version Instalada: {0}] [Version Anterior: {1}] Ventana de actualización deshabilitada".format(CONFIG.BUILDVERSION, version))
        else:
            logging.log("[Check Updates] [Version Instalada: {0}] [Version Anterior: {1}]".format(CONFIG.BUILDVERSION, version))
    else:
...
def check_sources():
    ...
    if len(match) > 0:
        ...
        if len(bad) > 0:
            choice = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]{1}[/COLOR][COLOR {2}] Source(s) Se han encontrado rotos".format(CONFIG.COLOR1, len(bad), CONFIG.COLOR2) + '\n' + "¿Le gustaría Eliminar todos o elegir uno por uno??[/COLOR]",
                                      yeslabel="[B][COLOR springgreen]Eliminar Todos[/COLOR][/B]",
            ...
            if choice == 1:
                ...
                for name, path, sharing, working in bad:
                    ...
                    if dialog.yesno(CONFIG.ADDONTITLE,
                                        ...
                                        yeslabel="[B][COLOR springgreen]Eliminar Source[/COLOR][/B]",
                                        nolabel="[B][COLOR red]Mantener Source[/COLOR][/B]"):

...
def show_save_data_settings():
    class FirstRun(xbmcgui.WindowXMLDialog):
        ...
        def onInit(self):
            ...
            self.thumbs = 313
            self.show_dialog()
            self.controllist = [self.trakt, self.alfa, self.login,
                                    ...
                                    self.thumbs]
            self.controlsettings = ['mantener trakt', 'mantener alfa', 'mantener login',
                                    'mantener sources', 'mantener profiles', 'mantener playercore', 'mantener guisettings', 'mantener advanced',
                                    'mantener favourites', 'mantener repos', 'mantener super',
                                    'mantenrwhitelist', 'Borrar cache', 'Borrar packages',
                                    'Borrar thumbs']
...
def show_build_prompt():
    class BuildPrompt(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.msg = "[B][COLOR white]Actualmente no hay Build instalada desde {0}[/COLOR][/B].\n\n[B][COLOR white]Seleccionar 'Menu de Instalacion' para instalar una Build de Kodileia Wizard o 'Ignorar' para no volver a ver este mensaje nunca más.[/COLOR][/B]\n\n[B][COLOR white]Gracias por escoger {1}.[/COLOR][/B]".format(CONFIG.ADDONTITLE, CONFIG.ADDONTITLE)
        ...
        def do_build_menu(self):
            logging.log("[Build Actual Check] [User Selected: Open Build Menu] [Next Check: {0}]".format(CONFIG.BUILDCHECK),
                        level=xbmc.LOGINFO)
...
def show_update_window(name='Testing Window', current='1.0', new='1.1', icon=CONFIG.ADDON_ICON, fanart=CONFIG.ADDON_FANART):
    class UpdateWindow(xbmcgui.WindowXMLDialog):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.fanart = kwargs['fanart']
            self.msgupdate = "Actualización disponible para el Pack Ajustes instalado:\n[COLOR {0}]{1}[/COLOR]\n\nVersión instalada: v[COLOR {2}]{3}[/COLOR]\nNueva versión: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}]*Recomendamos Instalacion Limpia[/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
            self.msgcurrent = "Ejecutando la última versión del Pack:\n[COLOR {0}]{1}[/COLOR]\n\nVersión instalada: v[COLOR {2}]{3}[/COLOR]\nNueva versión: v[COLOR {4}]{5}[/COLOR]\n\n[COLOR {6}][/COLOR]".format(CONFIG.COLOR1, self.name, CONFIG.COLOR1, self.current, CONFIG.COLOR1, self.new, CONFIG.COLOR1)
        ...
        def do_fresh_install(self):
            logging.log("[Check Updates] [Version Instalada: {0}] [Versión actual: {1}] [User Selected: Instalacion Limpia]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
        ...
        def do_normal_install(self):
            logging.log("[Check Updates] [Version Instalada: {0}] [Versión actual: {1}] [User Selected: Sin Limpieza]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST))
            logging.log("[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
        ...
        def do_ignore(self):
            logging.log("[Check Updates] [Version Instalada: {0}] [Versión actual: {1}] [User Selected: Ignorar {2} Dias]".format(CONFIG.BUILDVERSION, CONFIG.BUILDLATEST, CONFIG.UPDATECHECK))
            logging.log("[Check Updates] [Next Check: {0}]".format(tools.get_date(days=CONFIG.UPDATECHECK, formatted=True)))
            self.close()
    ...
    # update.doModal()
    # del update
    msgcurrent = 'Ejecutando la última versión del Pack Ajustes: '
    msgupdate = 'Actualización disponible del '
    build_name = '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, name)
    current_version = 'Versión instalada: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, current)
    latest_version = 'Nueva versión: v[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, new)
...
def show_notification(msg, test=False):
    class Notification(xbmcgui.WindowXMLDialog):
        ...
        def do_dismiss(self):
            ...
            logging.log('[Notifications] Notification {0} De Acuerdo'.format(CONFIG.get_setting('noteid')))
            self.close()
...
def show_log_viewer(window_title="Ver Log File", window_msg=None, log_file=None, ext_buttons=False):

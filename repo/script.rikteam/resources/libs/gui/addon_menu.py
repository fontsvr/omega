...
class AddonMenu:
    ...
    def show_menu(self, url=None):
        ...
        if url_response:
            ...
            TEMPADDONFILE = None
            logging.log("[Skins Menu] No hay skins agregados.")
        ...
        if TEMPADDONFILE:
            ...
            if addons_json:
                ...
                if addons and len(addons) > 0:
                    for addon in addons:
                        ...
                        if section:
                            directory.add_dir(addonname, {'mode': 'addons', 'url': addonurl}, description=description,
                                              icon=icon, fanart=fanart, themeit=CONFIG.THEME1)
                        else:
                            ...
                            if type.lower() == 'skin':
                                ...
                                try:
                                    ...
                                    if os.path.exists(add):
                                        addonname = "{0} [COLOR springgreen]- Instalado[/COLOR]".format(addonname)
                                except:
                                    pass
                    ...
                    if not addons:
                        directory.add_file('Formato de la lista no válido', themeit=CONFIG.THEME3)
                        logging.log("[Skin Menu] ERROR: Formato inválido.")
                    elif len(addons) == 0:
                        directory.add_file("No se agregaron elementos a este menú", themeit=CONFIG.THEME2)
        else:
            logging.log("[Skin Menu] ERROR: URL de la lista de skins no válida.")
            directory.add_file('Lista de Skins no disponible, prueba más tarde', themeit=CONFIG.THEME3)
    ...
    def install_addon_from_url(self, plugin, url):
        ...
        if not response:
            logging.log_notify("[COLOR {0}]Instalador de Skins[/COLOR]".format(CONFIG.COLOR1),
        ...
        self.progress_dialog.create(CONFIG.ADDONTITLE,
                               '[COLOR {0}][B]Descargando:[/B][/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2,
                                                                                                      CONFIG.COLOR1,
                                                                                                      plugin),
        ...
        title = '[COLOR {0}][B]Instalando:[/B][/COLOR] [COLOR {1}]{2}[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1,
                                                                                      plugin)
        self.progress_dialog.update(0, title, '', '[COLOR {0}]Espera[/COLOR]'.format(CONFIG.COLOR2))
        percent, errors, error = extract.all(lib, CONFIG.ADDONS, title=title)
        self.progress_dialog.update(0, title, '', '[COLOR {0}]Instalando dependencias[/COLOR]'.format(CONFIG.COLOR2))
        installed(plugin)
    ...
    def install_addon(self, plugin, urls, over=False):
        ...
        if not over:        
            if xbmc.getCondVisibility('System.HasAddon({0})'.format(plugin)):
                ...
                query = '{{"jsonrpc":"2.0","method":"Settings.SetSettingValue","id":1,"params":{{"setting":"lookandfeel.skin","value":{0}}}}}'.format(plugin)
        ...
        if False not in [repositoryxml_response, repositoryurl_response]:
            ...
            if not xbmc.getCondVisibility('System.HasAddon({0})'.format(repo_id)):
                logging.log("Repositorio no instalado, instalándolo")
                ...
                if version:
                    ...
                    logging.log(
                        "[Addon Installer] Repositorio no instalado: Imposible leer la url ({0})".format(urls[1]))
            else:
                logging.log("Repositorio instalado, instalando skin")
                install = install_from_kodi(plugin)
                if install:
                    xbmc.executebuiltin('Container.Refresh()')
                    plugin = '"{0}"'.format(plugin)
            ...
            logging.log("Sin repositorio, instalando skin")

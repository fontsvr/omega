    ...
    for root, dirs, files in os.walk(url):
        for file in files:
            ...
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
                progress_dialog.update(perc, "[COLOR {0}]Escanenado: [COLOR {1}]{2}[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, root.replace(CONFIG.HOME, '')) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, file) + '\n' + "Espera[/COLOR]")
...
def ascii_check(use=None, over=False):
    ...
    progress_dialog.create(CONFIG.ADDONTITLE, 'Espera...')
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            progress_dialog.update(prog2, "[COLOR {0}]Comprobando archivos no ASCII".format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, file) + '\n' + 'Espera[/COLOR]')
            try:
                file.encode('ascii')
    ...
    if total > 0:
        ...
        if yes:
            if use:
                logging.log_notify(CONFIG.ADDONTITLE,
                                 "[COLOR {0}]Comprobador ASCII: {1} Eliminado / {2} Fallido.[/COLOR]".format(CONFIG.COLOR2, f1, f2))
            else:
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]Comprobación ASCII: No Encontrados[/COLOR]".format(CONFIG.COLOR2))

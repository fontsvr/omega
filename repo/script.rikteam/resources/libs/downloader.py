        ...
        with open(dest, 'wb') as f:
            ...
            if not response:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Instalacion Build: Url Zip Invalida![/COLOR]'.format(CONFIG.COLOR2))
                return
            else:
            ...
            if total is None:
                ...
                for chunk in response.iter_content(chunk_size=max(int(total/512), mb)):
                    ...
                    currently_downloaded = '[COLOR %s][B]Tamaño:[/B] [COLOR %s]%.02f[/COLOR] MB de [COLOR %s]%.02f[/COLOR] MB[/COLOR]' % (CONFIG.COLOR2, CONFIG.COLOR1, downloaded / mb, CONFIG.COLOR1, total / mb)

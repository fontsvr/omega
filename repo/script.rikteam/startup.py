        ...
        if response:
            ...
            if repoversion:
                ...
                if repo_response:
                    ...
                    logging.log("[Auto Instalacion Repo] Instalado exitosamente", level=xbmc.LOGINFO)
                else:
                    logging.log_notify("[COLOR {0}]Repo Instalacion Error[/COLOR]".format(CONFIG.COLOR1),
                                       "[COLOR {0}]error URL zip![/COLOR]".format(CONFIG.COLOR2))
                    logging.log("[Auto Instalacion Repo] No se pudo crear una URL funcional para el repositorio. {0}".format(
            ...
            logging.log("[Auto Instalacion Repo] Unable to read the addon.xml file.", level=xbmc.LOGERROR)
    elif not CONFIG.AUTOINSTALL == 'Yes':
        logging.log("[Auto Instalacion Repo] Not Enabled", level=xbmc.LOGINFO)
    elif os.path.exists(os.path.join(CONFIG.ADDONS, CONFIG.REPOID)):
        logging.log("[Auto Instalacion Repo] Repositorio ya instalado")
    ...
    if note_id:
        if note_id == CONFIG.NOTEID:
            if CONFIG.NOTEDISMISS == 'false':
                ...
                logging.log('[Notifications] No hay nuevas notificaciones.', level=xbmc.LOGINFO)
...
def installed_build_check():
    ...
    if not CONFIG.EXTRACT == '100' and CONFIG.EXTERROR > 0:
        logging.log("[Instalacion Pack Check] Se extrajo el pack  {0}/100 with [ERRORS: {1}]".format(CONFIG.EXTRACT,
                                                                                                    CONFIG.EXTERROR),
                    level=xbmc.LOGINFO)
        ...
        if yes:
            ...
            logging.log("[Pack Installed Check] Instalacion limpia Re-activated", level=xbmc.LOGINFO)
        else:
            logging.log("[Pack Installed Check] Reinstall Ignored")
    elif CONFIG.SKIN in ['skin.confluence', 'skin.estuary', 'skin.estouchy']:
        logging.log("[Pack Instalado Check] Incorrect skin: {0}".format(CONFIG.SKIN), level=xbmc.LOGINFO)
        ...
        if not CONFIG.SKIN == defaults and not CONFIG.BUILDNAME == "":
            ...
            if not response:
                logging.log("[Pack Instalado Check] Guifix was set to http://", level=xbmc.LOGINFO)
                dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]Parece que la configuración del skin no se aplicó a la compilación.".format(CONFIG.COLOR2),
    ...
    if CONFIG.get_setting('installed') == 'true':
        if CONFIG.get_setting('Mantenertrakt') == 'true':
            from resources.libs import traktit
            logging.log('[Build Installed Check] Restaurar Trakt Data', level=xbmc.LOGINFO)
            traktit.trakt_it('restore', 'all')
        if CONFIG.get_setting('Manteneralfa') == 'true':
            from resources.libs import alfait
            logging.log('[Build Installed Check] Restaurar Real alfa Data', level=xbmc.LOGINFO)
            alfait.alfa_it('restore', 'all')
        if CONFIG.get_setting('Mantenerlogin') == 'true':
            from resources.libs import loginit
            logging.log('[Build Installed Check] Restaurar Login Data', level=xbmc.LOGINFO)
            loginit.login_it('restore', 'all')

        CONFIG.clear_setting('Instalar')
...
def build_update_check():
    ...
    if not response:
        ...
        logging.log("[Build Check] Build Instalada: Comprobar Actualizaciones", level=xbmc.LOGINFO)
...
def save_alfa():
    current_time = time.mktime(time.strptime(tools.get_date(formatted=True), "%Y-%m-%d %H:%M:%S"))
    next_save = time.mktime(time.strptime(CONFIG.get_setting('alfanextsave'), "%Y-%m-%d %H:%M:%S"))
    
    if next_save <= current_time:
        from resources.libs import alfait
        logging.log("[alfa Data] Guardar todos los datos", level=xbmc.LOGINFO)
        alfait.auto_update('all')
        CONFIG.set_setting('alfanextsave', tools.get_date(days=3, formatted=True))
    else:
        logging.log("[alfa Data] El siguiente guardado automático no es hasta: {0} / Hoy es: {1}".format(CONFIG.get_setting('alfanextsave'),
                                                                                           tools.get_date(formatted=True)),
                    level=xbmc.LOGINFO)
...
def auto_clean():
    ...
    if next_cleanup <= tools.get_date() or freq == 0:
        ...
        logging.log("[Auto Clean Up] Siguiente limpieza {0}".format(CONFIG.NEXTCLEANDATE),
                    level=xbmc.LOGINFO)
    if service:
...
# FIRST RUN SETTINGS
if CONFIG.get_setting('Primera_Instalacion') == 'false':
    logging.log("[First Run] Mostrando configuración de guardar datos", level=xbmc.LOGINFO)
    window.show_save_data_settings()
else:
    logging.log("[First Run] Omitir la configuración de guardar datos", level=xbmc.LOGINFO)
...
if CONFIG.get_setting('buildname'):
    ...
    if current_time >= epoch_check:
        logging.log("[Build Update Check] Empezando", level=xbmc.LOGINFO)
        build_update_check()
else:
    logging.log("[Build Update Check] Siguiente chequeo: {0}".format(buildcheck), level=xbmc.LOGINFO)

# AUTO INSTALL REPO
if CONFIG.AUTOINSTALL == 'Yes':
    logging.log("[Auto Install Repo] Empezando", level=xbmc.LOGINFO)
    auto_install_repo()
else:
    logging.log("[Auto Install Repo] Siguiente chequeo", level=xbmc.LOGINFO)

# REINSTALL ELIGIBLE BINARIES
binarytxt = os.path.join(CONFIG.USERDATA, 'build_binaries.txt')
if os.path.exists(binarytxt):
    logging.log("[Binary Detection] Reinstalación de complementos binarios elegibles", level=xbmc.LOGINFO)
    from resources.libs import restore
    restore.restore('binaries')
else:
    logging.log("[Binary Detection] Complementos binarios elegibles para reinstalar", level=xbmc.LOGINFO)
    
# AUTO UPDATE WIZARD
if CONFIG.AUTOUPDATE == 'Yes':
    logging.log("[Auto actualizar Wizard] Started", level=xbmc.LOGINFO)
    update.wizard_update()
else:
    logging.log("[Auto actualizar Wizard] Not Enabled", level=xbmc.LOGINFO)
...
# SAVE TRAKT
if CONFIG.get_setting('Mantener_trakt') == 'true':
...
# SAVE alfa
if CONFIG.get_setting('Mantener_alfa') == 'true':
    logging.log("[alfa Data] Started", level=xbmc.LOGINFO)
    save_alfa()
else:
    logging.log("[alfa Data] Not Enabled", level=xbmc.LOGINFO)

# SAVE LOGIN
if CONFIG.get_setting('Mantener_login') == 'true':

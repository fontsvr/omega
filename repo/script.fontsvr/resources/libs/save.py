...
def import_save_data():
    ...
    if not os.path.exists(TEMP):
        os.makedirs(TEMP)
    source = dialog.browse(1, '[COLOR {0}]Selecionar directorio SaveData.zip[/COLOR]'.format(CONFIG.COLOR2),
            ...
            if os.path.exists(old):
                if overwrite == 1:
                    ...
                    if not dialog.yesno(CONFIG.ADDONTITLE,
                                            "[COLOR {0}]¿Le gustaría reemplazar [COLOR {1}]{2}[/COLOR] archivo?".format(CONFIG.COLOR2, CONFIG.COLOR1, item),
                                            yeslabel="[B][COLOR springgreen]Yes Reemplazar[/COLOR][/B]",
                                            nolabel="[B][COLOR red]No[/COLOR][/B]"):
                        continue
                    else:
                        os.remove(old)
    ...
    if os.path.exists(alfa):
        from resources.libs import alfait

        x += 1
        files = os.listdir(alfa)
        if not os.path.exists(CONFIG.alfaFOLD):
            os.makedirs(CONFIG.alfaFOLD)
        for item in files:
            old = os.path.join(CONFIG.alfaFOLD, item)
            temp = os.path.join(alfa, item)
            if os.path.exists(old):
                if overwrite == 1:
                    os.remove(old)
                else:
...
def export_save_data():
    ...
    dir = ['alfa', 'login', 'trakt']
    keepx = [CONFIG.KEEPADVANCED, CONFIG.KEEPSOURCES, CONFIG.KEEPFAVS, CONFIG.KEEPPROFILES, CONFIG.KEEPMYVIDEOS, CONFIG.KEEPPLAYERCORE, CONFIG.KEEPGUISETTINGS]
    traktit.trakt_it('update', 'all')
    loginit.login_it('update', 'all')
    alfait.alfa_it('update', 'all')

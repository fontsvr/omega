...
def cleanup_backup():
    ...
    for item in sorted(folder, key=os.path.getmtime):
        ...
        if os.path.isdir(item):
            ...
            list.append(base)
    list = ['--- Borrar todos los zips ---'] + list
    selected = dialog.select("{0}: Elige las copias a borrar.".format(CONFIG.ADDONTITLE),
                             list)
    ...
    if selected == -1:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Quieres eliminar todos las copias de Seguridad?[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, CONFIG.MYBUILDS),
                        yeslabel="[B][COLOR springgreen]Eliminarlas[/COLOR][/B]",
                        nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
            clearedfiles, clearedfolders = tools.clean_house(CONFIG.MYBUILDS)
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Archivos Borrados: [COLOR {1}]{2}[/COLOR] / Carpetas:[/COLOR] [COLOR {3}]{4}[/COLOR]".format(
        ...
        if dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Quieres Eliminar [COLOR {1}]{2}[/COLOR] de la carpeta de Copias de Seguridad?[/COLOR]".format(CONFIG.COLOR2, CONFIG.COLOR1, list[selected]) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, path),
                        yeslabel="[B][COLOR springgreen]Eliminarlas[/COLOR][/B]",
                        nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
            ...
            if passed:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]{1} Eliminados[/COLOR]".format(CONFIG.COLOR2, list[selected]))
            else:
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Error al borrar {1}![/COLOR]".format(CONFIG.COLOR2, list[selected]))
        else:
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Limpieza Cancelada![/COLOR]".format(CONFIG.COLOR2))
...
class Backup:
    ...
    def backup_build(self, name=""):
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                             "[COLOR {0}]Estás seguro de que deseas hacer una copia de seguridad de la Build Actual?[/COLOR]".format(CONFIG.COLOR2), nolabel="[B][COLOR red]Cancel Backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup Build[/COLOR][/B]"):
            if name == "":
                name = tools.get_keyboard("", "Introduce Nombre del Zip")
                if not name:
                    return False
            ...
            if len(binaries) > 0:
                self.dialog.ok(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Los siguientes complementos se excluyeron de la copia de seguridad porque son específicos de la plataforma:[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, ', '.join(binaries)))
    ...
    def backup_gui(self, name=""):
        if name == "":
            guiname = tools.get_keyboard("", "Introduce un nombre para la Copia de Seguridad")
            if not guiname:
                return False
        ...
        tempguizipname = ''
        guizipname = os.path.join(CONFIG.MYBUILDS, '{0}_backup.zip'.format(guiname))
        if os.path.exists(CONFIG.GUISETTINGS):
            try:
                zipf = zipfile.ZipFile(guizipname, mode='w', allowZip64=True)
            except:
                try:
                    tempguizipname = os.path.join(CONFIG.PACKAGES, '{0}_backup.zip'.format(guiname))
                    zipf = zipfile.ZipFile(tempguizipname, mode='w', allowZip64=True)
                except:
                    logging.log("Unable to create {0}_backu.zip".format(guiname), level=xbmc.LOGERROR)
                    if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]No se puede escribir en el directorio escogido, Quieres cambiar el directorio?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[COLOR springgreen]Cambiar Directorio[/COLOR]",
                        ...
                        return
            try:
                zipf.write(CONFIG.GUISETTINGS, 'Database/MyVideos119.db', zipfile.ZIP_DEFLATED)
        ...
        if name == "":
            self.dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Copia de Seguridad de la Videoteca guardada en:[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, guizipname))

    def backup_skin(self, name=""):
        if name == "":
            guiname = tools.get_keyboard("", "Introduce un nombre para la Copia de Seguridad del Skin")
            if not guiname:
                return False
            tools.convert_special(CONFIG.USERDATA, True)
        else:
        ...
        guiname = quote_plus(guiname)
        tempguizipname = ''
        guizipname = os.path.join(CONFIG.MYBUILDS, '{0}_skinbackup.zip'.format(guiname))
        if os.path.exists(CONFIG.GUISETTINGS):
            try:
                zipf = zipfile.ZipFile(guizipname, mode='w', allowZip64=True)
            except:
                try:
                    tempguizipname = os.path.join(CONFIG.PACKAGES, '{0}_skinbackup.zip'.format(guiname))
                    zipf = zipfile.ZipFile(tempguizipname, mode='w', allowZip64=True)
                except:
                    logging.log("Unable to create {0}_skinbackup.zip".format(guiname), level=xbmc.LOGERROR)
                    if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]No se puede escribir en el directorio escogido, Quieres cambiar el directorio?[/COLOR]".format(CONFIG.COLOR2),
                                         yeslabel="[COLOR springgreen]Cambiar Directorio[/COLOR]",
                                         nolabel="[COLOR red]Cancel[/COLOR]"):
                        CONFIG.open_settings()
            ...
            logging.log("[Back Up] Type = guifix: guisettings.xml not found")
        if name == "":
            self.dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Copia de Seguridad del Skin guardada en:[/COLOR]".format(CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, guizipname))

    def backup_theme(self, name=""):
        if not self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Copia de Seguridad del Skin[/COLOR]'.format(CONFIG.COLOR2), "[COLOR {0}]Quieres crear una copia de seguridad del skin?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[B][COLOR springgreen]Continuar[/COLOR][/B]",
                                 nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
            logging.log_notify("Copia del Skin", "Cancelada!")
            return False
        if name == "":
            themename = tools.get_keyboard("", "Introduce un nombre para el zip")
            if not themename:
                return False
        else:
            themename = name
        ...
        try:
            ...
            try:
                ...
                if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]No ha sido posible escribir el archivo en la ubicación actual, quieres cambiar la ubicación?[/COLOR]".format(CONFIG.COLOR2),
                                     yeslabel="[B][COLOR springgreen]Cambiar Directorio[/COLOR][/B]",
                                     nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
                    CONFIG.open_settings()
                    return
        ...
        try:
            ...
            if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Copia de Seguridad del Skin[/COLOR]'.format(CONFIG.COLOR2),
                                 "[COLOR {0}]Quieres incluir algún addon?[/COLOR]".format(CONFIG.COLOR2),
                                 yeslabel="[B][COLOR springgreen]Sí, Incluir[/COLOR][/B]",
                            ...
                            for depends in match:
                                ...
                                for base, dirs, files in os.walk(os.path.join(CONFIG.ADDONS, depends)):
                                    ...
                                    for file in files:
                                        ...
                                        added.append(depends)
            if self.dialog.yesno(CONFIG.ADDONTITLE + '[COLOR {0}]: Copia de Seguridad del Skin[/COLOR]'.format(CONFIG.COLOR2),
                                 "[COLOR {0}]Quieres incluir el archivo [COLOR {1}]guisettings.xml[/COLOR] que contiene los Ajustes de la Aplicación?[/COLOR]".format(
        ...
        if not tempzipname == '':
            ...
            if success == 0:
                xbmcvfs.copy(tempzipname, zipname)
                xbmcvfs.delete(tempzipname)
        self.dialog.ok(CONFIG.ADDONTITLE, "Copia de Seguridad del Skin [COLOR {0}]{1}[/COLOR][COLOR {2}] guardada en:[/COLOR]".format(CONFIG.COLOR1, themename, CONFIG.COLOR2) + '\n' + "[COLOR {0}]{1}[/COLOR]".format(CONFIG.COLOR1, zipname))

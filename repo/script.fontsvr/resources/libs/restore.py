...
def binaries():
    ...
    if os.path.exists(binarytxt):
        ...
        dialog.ok(CONFIG.ADDONTITLE,
                  '[COLOR {0}]La Build restaurada contiene complementos específicos de la plataforma, que ahora serán '
...
class Restore:
    ...
    def _from_file(self, file, loc):
        ...
        self.progress_dialog.update(0, 'Instalando External Backup' + '\n' + 'Espere')
        ...
        tools.kill_kodi(
            msg='[COLOR {0}]Para aplicar los cambios, es necesario forzar el cierre de la Aplicación. Cerramos?[/COLOR]'.format(
                CONFIG.COLOR2))
    ...
    def choose(self, location):
        ...
        file = self.dialog.browseSingle(1, '[COLOR {0}]Seleccione el archivo de respaldo que desea restaurar[/COLOR]'.format(
        ...
        if not file.endswith('.zip'):
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]{1} Restore: Cancelado[/COLOR]".format(
...
def restore(action, external=False):
    ...
    elif action in ['gui', 'theme', 'addonpack', 'skin']:

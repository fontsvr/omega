    ...
    if response:
        ...
        if ver > CONFIG.ADDON_VERSION:
            yes = dialog.yesno(CONFIG.ADDONTITLE,
                                   ...
                                   'Te gustaria descargar [COLOR {0}]v{1}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, ver),
                                   nolabel='[B][COLOR red]Recuérdamelo más tarde[/COLOR][/B]',
                                   yeslabel="[B][COLOR springgreen]Actualizar Wizard[/COLOR][/B]")
            if yes:
                from resources.libs import db
                from resources.libs.common import tools

                logging.log("[Auto Update Wizard] Instalando Wizard v{0}".format(ver))
                progress_dialog.create(CONFIG.ADDONTITLE, '[COLOR {0}]Descargando Actualizacion...'.format(CONFIG.COLOR2), '',
                ...
                logging.log_notify(CONFIG.ADDONTITLE,
                                   '[COLOR {0}]Add-on actualizada[/COLOR]'.format(CONFIG.COLOR2))
                logging.log("[Auto Update Wizard] Wizard actualizada v{0}".format(ver))
                tools.remove_file(os.path.join(CONFIG.ADDON_DATA, 'settings.xml'))
                window.show_save_data_settings()
            else:
                logging.log("[Auto Update Wizard] Intalacion de wizard Ignorado: {0}".format(ver))
        else:
            logging.log("[Auto Update Wizard] No hay nueva Version v{0}".format(ver))
    else:
        logging.log("[Auto Update Wizard] Url del wizard no es Valida: {0}".format(CONFIG.BUILDFILE))
...
def toggle_addon_updates():
    ...
    setting = '"general.addonupdates"'
    selected = 0
    options = ['Install updates automatically', 'Notify, but don\'t install updates', 'Nunca comprobar si hay actualizaciones']

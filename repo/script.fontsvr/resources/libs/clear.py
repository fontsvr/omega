                ...
                if file_count > 0:
                    ...
                    if yes:
                        ...
                        logging.log_notify(CONFIG.ADDONTITLE,
                                  '[COLOR {0}]Paquetes Borrados[/COLOR]'.format(CONFIG.COLOR2))
                else:
                    logging.log_notify(CONFIG.ADDONTITLE,
                              '[COLOR {0}]No hay paquetes para borrar[/COLOR]'.format(CONFIG.COLOR2))
...
def clear_cache(over=None):
    ...
    if CONFIG.INCLUDEVIDEO == 'true' and over is None:
        ...
        if len(files) > 0:
            ...
            logging.log("Clear Cache: Clear Video Cache Not Enabled")
    logging.log_notify(CONFIG.ADDONTITLE,
                       '[COLOR {0}]Borrar Caché: {1} Archivo(s) Borrado(s)[/COLOR]'.format(CONFIG.COLOR2, delfiles))
...
def total_clean():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        '[COLOR {0}]Quieres limpiar el caché, paquetes e imágenes?[/COLOR]'.format(CONFIG.COLOR2),
                        nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',
                        yeslabel='[B][COLOR springgreen]Sí, Limpiar[/COLOR][/B]'):
        clear_archive()
        clear_cache()
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Quieres Eliminar [COLOR {1}]TODOS[/COLOR] los Datos del addon en la carpeta addon_data?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
            ...
            logging.log_notify('[COLOR {0}]Borrar Datos[/COLOR]'.format(CONFIG.COLOR1),
                               '[COLOR {0}]Cancelado[/COLOR]'.format(CONFIG.COLOR2))
    elif addon == 'uninstalled':  # clear addon data for uninstalled addons
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Quieres Eliminar [COLOR {1}]TODOS[/COLOR] los Datos de addons desinstalados?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[B][COLOR springgreen]Eliminar Datos[/COLOR][/B]',
                            nolabel='[B][COLOR red]No Eliminar[/COLOR][/B]'):
            ...
            for folder in glob.glob(os.path.join(CONFIG.ADDON_DATA, '*')):
                ...
                if foldername in CONFIG.EXCLUDES:
                    ...
                    logging.log(folder)
                    shutil.rmtree(folder)
            logging.log_notify('[COLOR {0}]Eliminar Desinstalados[/COLOR]'.format(CONFIG.COLOR1),
                               '[COLOR {0}]{1} Carpeta(s) Eliminada(s)[/COLOR]'.format(CONFIG.COLOR2, total))
        else:
            logging.log_notify('[COLOR {0}]Borrar Datos[/COLOR]'.format(CONFIG.COLOR1),
                               '[COLOR {0}]Cancelado[/COLOR]'.format(CONFIG.COLOR2))
    elif addon == 'empty':  # clear empty folders from addon_data
        if dialog.yesno(CONFIG.ADDONTITLE,
                            '[COLOR {0}]Quieres Eliminar [COLOR {1}]TODAS[/COLOR] las carpetas de Datos vacías?[/COLOR]'.format(CONFIG.COLOR2, CONFIG.COLOR1),
                            yeslabel='[B][COLOR springgreen]Sí, Borrar[/COLOR][/B]',
                            nolabel='[B][COLOR red]No Borrar[/COLOR][/B]'):
            total = tools.empty_folder(CONFIG.ADDON_DATA)
            logging.log_notify('[COLOR {0}]Borrar carpetas vacias[/COLOR]'.format(CONFIG.COLOR1),
                               '[COLOR {0}]{1} Carpeta(s) Eliminada(s)[/COLOR]'.format(CONFIG.COLOR2, total))
        else:
            logging.log_notify('[COLOR {0}]Borrar Carpetas Vacías[/COLOR]'.format(CONFIG.COLOR1),
                               '[COLOR {0}]Cancelado[/COLOR]'.format(CONFIG.COLOR2))
        ...
        if addon in CONFIG.EXCLUDES:
            ...
            if dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Quieres eliminar también los datos del addon:[/COLOR]'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, addon), yeslabel='[B][COLOR springgreen]Eliminar Datos[/COLOR][/B]', nolabel='[B][COLOR red]No Borrar Datos[/COLOR][/B]'):
...
def remove_addon_menu():
    ...
    if len(selected) > 0:
        ...
        dialog.ok(CONFIG.ADDONTITLE, "[COLOR {0}]Para aplicar los cambios es necesario reiniciar la aplicación, pulsa OK para cerrarla.[/COLOR]".format(CONFIG.COLOR2))

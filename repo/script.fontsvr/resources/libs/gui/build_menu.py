...
class BuildMenu:
    ...
    def _list_all(self, match, kodiv=None):
        ...
        for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
            ...
            if not kodiv or kodiv == int(float(kodi)):
                ...
                directory.add_dir('{0} (v{1})'.format(name, version), {'mode': 'install', 'action': 'build', 'name': name}, description=description, fanart=fanart, icon=icon, menu=menu, themeit=CONFIG.THEME1)
    ...
    def view_build(self, name):
        ...
        for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
            ...
            directory.add_dir('Menu Salvar Datos', {'mode': 'savedata'}, icon=CONFIG.ICONSAVE, themeit=CONFIG.THEME3)
            directory.add_file('Informacion de la Build', {'mode': 'buildinfo', 'name': name}, description=description, fanart=fanart,
                               icon=icon, themeit=CONFIG.THEME3)
                               
            if previewcheck:
                directory.add_file('Ver Video', {'mode': 'buildpreview', 'name': name}, description=description, fanart=fanart,
                                   icon=icon, themeit=CONFIG.THEME3)
            
            if versioncheck:
                directory.add_file(
                    '[I]Build Diseñada para Kodi v{0} (installed: v{1})[/I]'.format(str(kodi), str(CONFIG.KODIV)),
                    fanart=fanart, icon=icon, themeit=CONFIG.THEME3)
                    
            directory.add_separator('INSTALACION')

...
def whitelist(do):
    ...
    if do == 'edit':
        ...
        while choice not in [-1, 0]:
            choice = dialog.select("{0}: Seleccione los complementos que desea incluir en la lista blanca.".format(CONFIG.ADDONTITLE), tempaddonnames)
            if choice == -1:
                break
            elif choice == 0:
                break
        ...
        logging.log_notify(CONFIG.ADDONTITLE,
                           "[COLOR {0}]{1} Add-ons en Lista Blanca[/COLOR]".format(CONFIG.COLOR2, len(selected)))
    elif do == 'read':
        white_list = []
        ...
        if len(list) > 0:
            msg = "Aquí hay una lista de los elementos de su lista blanca, estos elementos(junto con las dependencias) no se eliminará cuando se realice un nuevo comienzo o los datos del usuario se sobrescriban en una instalación de Build.[CR][CR]"
            for item in list:
                try:
                    name, id, fold = item
                except Exception as e:
        ...
        if not source.endswith('.txt'):
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Cancelada[/COLOR]".format(CONFIG.COLOR2))
            return
        ...
        try:
            ...
            dialog.ok(CONFIG.ADDONTITLE,
                          "[COLOR {0}]La lista blanca se ha exportado a:[/COLOR]".format(CONFIG.COLOR2),
            ...
            if not dialog.yesno(CONFIG.ADDONTITLE,
                                    ...
                                    yeslabel="[B][COLOR springgreen]Cambiar Localizacion[/COLOR][/B]",
                                    nolabel="[B][COLOR red]No Cancelar[/COLOR][/B]"):
                logging.log_notify(CONFIG.ADDONTITLE,
                                   "[COLOR {0}]Exportacion de Lista Blanca Cancelada[/COLOR]".format(CONFIG.COLOR2, e))
            else:
                whitelist(do='export')
    elif do == 'clear':
        if not dialog.yesno(CONFIG.ADDONTITLE,
